...
const setEntry = async () => {
  const response = await fetch('http://localhost:1975/user/'+route.params.id)
...
const updateUser = async () => {
  await useFetch('http://localhost:1975/user/update', {
    method: 'PUT',
    body: {
...
<template>
  <form @submit.prevent="updateUser" class="w-full max-w-md mx-auto">
    <h3 class="text-lg font-semibold mb-4">Edit User {{ route.params.id }} </h3>
    ...
    <div class="flex items-center">
      <button type="submit" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded mr-1">Update</button>
      <nuxt-link to="/" class="inline-block align-baseline font-bold text-sm text-yellow-500 hover:text-yellow-800" >Go Back</nuxt-link>
    </div>
  </form>
</template>